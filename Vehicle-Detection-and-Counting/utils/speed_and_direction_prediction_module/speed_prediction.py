...
def predict_speed(
    ...
    scale_constant = 1  # manual scaling because we did not perform camera calibration
    isInROI = True  # is the object inside Region Of Interest
    update_csv = False

    if bottom < 250:
        scale_constant = 1  # scale_constant is used for manual scaling because we did not perform camera calibration
    elif 250 < bottom < 320:
        scale_constant = 2  # scale_constant is used for manual scaling because we did not perform camera calibration
    else:
        isInROI = False

    if len(bottom_position_of_detected_vehicle) != 0 and bottom \
        - bottom_position_of_detected_vehicle[0] > 0 and 195 < bottom_position_of_detected_vehicle[0] < 240 \
    ...
    if isInROI:
        ...
        scale_real_length = pixel_length * 44  # multiplied by 44 to convert pixel length to real length in meters (
        # change 44 to get length in meters for your case)
        total_time_passed = current_frame_number - current_frame_number_list[0]
        scale_real_time_passed = total_time_passed * 24  # get the elapsed total time for a vehicle to pass through
        # ROI area (24 = fps)
        if scale_real_time_passed != 0:
            speed = scale_real_length / scale_real_time_passed / scale_constant  # performing manual scaling because
            # we have not performed camera calibration
